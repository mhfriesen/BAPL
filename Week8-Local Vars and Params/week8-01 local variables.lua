...
local take, split, kpairs = base.take, base.split, base.kpairs
...
local local_capture = #(ID * space) *  -- is local var 
    function(s,p) pp = p; return true end *  -- matchtime save ID position in pp
    lpeg.C(ID) *                             -- capture ID
    function(s,p)                            -- matchtime syntax/semantic checks and register var                                    
      local id = string.sub(s, pp, p - 1)
...
local var_use = #(ID) * function(s,p) pp = p; return true end * lpeg.C(ID) *
    function(s,p)
      ...
      if not isDefined then
        print("not defined var_use id=", id)
        return false 
      else
        return true
      end
...
local function nodeFunction(name, params, body)
  --print("nodeFunction-----name ", name)
  --print("params=", pt.pt(params))
  --print("body=", pt.pt(body))
  ...
  local tree = {tag = "function", name = name, body = fwd_funcs_body[name], params = params}
  --print("tree", pt.pt(tree))
  --print("--type(tree.params)=", type(tree.params))
  return tree
end
...
local grammar = P{"prog",
  ...
  * space * T"(" * params * T")",
  ...
  params = lpeg.Ct( (param_capture * (T"," * param_capture)^0 )^-1 ),
  ...
  call_capture = #(ID * space * T"(") *     -- is name 
    ...
    function(s,p)                           -- matchtime syntax/semantic checks and register var                                    
      ...
      elseif not fwd_funcs[id] then
        ...
        --print("call func id=", id)
        return true
      end
    end
  * space * T"(" * args * T")",
  
  args = lpeg.Ct( ( interp * (T"," * interp)^0 )^-1 ),
...
function Compiler:findLocal(name)
  ...
  --print("name", "'"..name.."'", #locs, "locs=", pt.pt(locs))
  -- is name a local var
  for i = #locs, 1, -1 do
    --print("i",i,locs[i])
    if locs[i] == name then
  ...
  for i = #params, 1, -1 do
    ...
    if params[i] == name then
      return -(#params - i) -- put on stack before base of locals so idx backward
    end
  end
  return nil
end
...
function Compiler:codeCall(ast)
  ...
  local args = ast.args
  --print("-----------checking args for ", ast.fname)
  --print("func", pt.pt(func))
  --print("ast", pt.pt(ast))
  --print("args", pt.pt(func.args))
  if #func.params ~= #args then
    error("wrong number of arguments calling function " .. ast.fname)
  end
  
  --print("-----------coding call for ", ast.fname)
  --print("args", pt.pt(args))
  for i = 1, #args do
    self:codeExp(args[i])
  end
  self:addCode("call")
...
function Compiler:codeExp(ast)
  ...
  elseif ast.tag == "variable" then
    ...
    --print("ast.var=", "'"..ast.var.."'", "idx=",idx, "locals=", pt.pt(self.locals))
    if idx then
      self:addCode("loadL")
      self:addCode(idx)
    else
...
function Compiler:codeBlock(ast)
  Compiler.oldlevel = #self.locals
  self:codeStmt(ast.body)
  -- remove locals associated with this block
  local diff = #self.locals - Compiler.oldlevel
  if 0 < diff then
...
local function run(code, mem, stack, top, options)
  ...
  while true do 
    ...
    elseif code[pc] == "loadL" then 
      ...
      local id = code[pc]
      top = top + 1
      --print("id", id, "base" ,base , stack[base + id])
...
print"\nBeautifully Arranged Pedalogical Language  pl -- aka BAPL\n"
print("NEW cases")

function CASES() end -- for IDE outline view
local optTrace, n1, n2 = false, 1, 2
cases = {
[[
function fact(n) {
  if n {
...
[[
#{
in the VM my zone type is a table
after adding a single line in function tozone(t)
   t["size"] = 2 
BOOM! my zone type inherited all the functionality of my array type :)
}#
function main() {
  z = 3:9 + 1:-7;  
  @ z;
