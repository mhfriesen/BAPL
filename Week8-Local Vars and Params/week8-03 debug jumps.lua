...
function Compiler:codeJmpB(op, iPC)
  ...
  self:addCode(diff + 1) -- jump backward to pc == iPC
end
...
local function run(code, mem, stack, top, options)
  ...
  while true do 
    ...
    elseif code[pc] == "jmp" then 
      pc = pc + code[pc+1] - 1 -- unconditional jump 
...
function init(a, r, c) {
  ...
  #a_nr, a_nc = init(a, 4, 4);
  #@ "dims of a are {} rows {} cols"  <- a_nr  <- a_nc;
]]
,
[[
...
function main () {
  ...
  @ "n_isplaceok_called = {}"  <- n_isplaceok_called;
  @ "\n"
}
...
function isplaceok (rows_col, cur_row, col) {
 ...
 function printsolution (rows_col) {
  ...
  var row = 1;
  while row <= rows_col["size"] {
   var col = 1;
   while col <= rows_col["size"] {
    @ rows_col[row] == col and "X" or "_";
    @ " ";
    col = col + 1
   };
   @ "\n";
...
function write_rows_col(rows_col) {
  var i = 1;
  while i <= rows_col["size"] {    #---------------- check that size lookup works; if not implement it
    @ rows_col[i] | " ";
    i = i + 1;
  }
}
  ...
  if cur_row > N {
    ...
    while col <= N {
      if isplaceok(rows_col, cur_row, col) {
        ...
        if 1 or not isa_soln_printed { #prevent backtracking on other columns after first soln is found
