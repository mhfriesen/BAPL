...
--[[
take {'a',  {val = 'k1', tag = 'string'}, {val = 'k2', tag = 'string'  } ...}  arrrays
                                         {val = 1, tag = "number"}            associative arrays
return
{tag = 'indexed',
 array = {
   tag = 'indexed'},
...
local function foldIndexed_use(lst)
  ...
  --log("--foldIndexed_use tree=", pt.pt(tree))
  return tree
end
...
local function multivar_assgn(lst) --used for destructuring arrays
  ...
  -- wrap indexing around array for each  multiple var 
  local assigns = { }
  for i = 1 , #lst-1 do
    assigns[#assigns + 1] = {tag = "assgn", 
                          lhs = {tag = "variable", var = lst[i]}, -- first of the multiple vars
                          expr = {tag = "indexed", array = array, index = {tag = "number", val = i}}  
                        } 
  end
  --log("\n--assigns=", pt.pt(assigns))
  -- now we need assigns as nested {assign1, assign2}, assign3} to be applied two at time to nodeSeq
  local tree = foldAssigns(assigns) 
  --log("\n--tree=", pt.pt(tree))
  return tree
end

local function multivar_local_assgn(lst) --used for destructuring arrays
  -- lower to a sequence of indexed assignments
  -- for associative arrays the index number will correspond to the i'th iterated key
  --log("----multivar_assgn lst=", pt.pt(lst))
  local array = lst[#lst]
  ...
  local assigns = { }
  for i = 1 , #lst-1 do
    assigns[#assigns + 1] = {tag = "local", 
                          name =  lst[i], -- first of the multiple vars
...
local function multivar_local_assgn_old(lst) --used for destructuring arrays
  -- for associative arrays the index number will correspond to the i'th iterated key
  
  -- lower to a sequence of indexed local assignments node("local", "name", "init")
  --log("multivar_local_assgn lst=", pt.pt(lst))
  local array = lst[#lst].var
  --log("\narray=", array)
  local tree = nodeSeq {tag = "local", name = lst[1], init = foldIndexed_use({array, {tag = "number", val = 1}})  }
  
  for i = 2 , #lst-1 do
    tree = nodeSeq( tree, {tag = "local", name = lst[i], init = foldIndexed_use({array, {tag = "number", val = i}}) } )
  end
  --log("\nseq=", pt.pt(tree))
  return tree
end
...
local grammar = P{"prog",
  ...
  stmt = 
      ...
      + Rw"var" * multivar_capture * Ct(lpeg.C(ID) * (T"," * lpeg.C(ID) * space)^1 * T"="  * interp)
        / multivar_local_assgn
      + Rw"var" * local_capture * ( (T"=" * interp) + lpeg.Cc("nil") ) / node("local", "name", "init")
      + Rw"for" * Ct(var_capture * interp * T"," * interp * (T"," * interp)^-1  * block) / composeFor * space 
  ...
  factor =  
      ...
      + T"(" * interp * T")" 
      + Ct(indexed_use * (T"[" * interp * T"]")^0) / foldIndexed_use -- for md arrays fold indexing
      + T"nil"
      + call
...
function Compiler:codeStmt(ast)
  ...
  elseif ast.tag == "local" then
    ...
    -- TODO: check that var is not being declared again
...
# enhanced with 'unless' and 'string interpolation'
...
# and is higher priority than or so no parenthesese needed
# but, can also use while (expr) { } without confusing with a function call
function main () {
    var i = 6;
    var to = 0;
    var step = -2;
