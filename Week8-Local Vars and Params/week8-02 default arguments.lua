...
local take, split, kpairs, factorial = base.take, base.split, base.kpairs, base.factorial
local log = print
...
function Compiler:findLocalIndex(name)
  ...
  --log("name", name, "self.default_lookup_flag", self.default_lookup_flag)
  --if not self.default_lookup_flag then
  ...
  if #params + #def_params == 0 then return nil end
  
  for i = #params + #def_params, #params, -1 do
...
function Compiler:codeJmpB(op, iPC)
  self:addCode(op)
  self:addCode(iPC - self:currentPC() - 1) -- relative jmp backward
end

function Compiler:codeJmpF(op)
  self:addCode(op)
  self:addCode(self:currentPC() ) -- to be updated later relative to the pos coded here
  return self:currentPC()
end

function Compiler:fixJmp2here(jmp)
  self.code[jmp] = self:currentPC() - self.code[jmp] + 1  -- relative jump forward
end
...
local function run(code, mem, stack, top, options)
  ...
  while true do 
    ...
    elseif code[pc] == "jmp" then 
      --log("--------jmp pc=", pc, "code[pc+1]=",code[pc+1])
      --log(pc + code[pc+1])
      pc = pc + code[pc+1] + 1 -- unconditional relative jump
    elseif code[pc] == "jmpZ" then 
      pc = pc + 1
      if stack[top] == 0 or stack[top] == nil then -- conditional
        pc = pc + code[pc] - 2 -- relative jump
      end
      top = top - 1 -- take condition off stack
    elseif code[pc] == "jmpA" then  -- if not jumping take the cond off the stack since the next expr will give leave it
      pc = pc + 1 
      if stack[top] == 0 or stack[top] == nil then -- conditional
        pc = pc + code[pc] - 2 -- relative jump
      else
...
local optTrace, n1, n2 = true, 11, 11
cases = {
[[
