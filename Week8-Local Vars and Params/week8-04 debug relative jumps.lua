...
function Compiler:currentPC()
  return #self.code -- offset by any relative code movement
end

function Compiler:codeJmpB(op, iPC)
  self:addCode(op)
  self:addCode(iPC) -- jump backward to pc == iPC
end

function Compiler:codeJmpF(op)
  self:addCode(op)
  self:addCode(0) -- to be updated later using the pc returned
  return self:currentPC() 
end

function Compiler:fixJmp2here(jmp)
  self.code[jmp] = self:currentPC()  
end
...
local function run(code, mem, stack, top, options)
  ...
  while true do 
    ...
    elseif code[pc] == "jmp" then 
      pc = code[pc+1]  -- unconditional jump TODO: relative
    elseif code[pc] == "jmpZ" then 
      pc = pc + 1
      if stack[top] == 0 or stack[top] == nil then -- conditional
        pc = code[pc]  -- TODO: code[pc] was compiled to relative jump
      end
...
local optTrace, n1, n2 = false, 1, 99
cases = {
[[
