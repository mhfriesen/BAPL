...
dofile(lfs.currentdir().."\\MHF-CAPL-Course\\pt.lua") --table to string
dofile(lfs.currentdir().."\\MHF-CAPL-Course\\test.lua") -- run use cases
loc = lpeg.locale()
...
local number = (scientific + hex + decimal) / tonumber * space -- tonumber for testing
...
test("numbers",number,{
    ...
    "1e-10",
    "0xAf",
    "0xAfX", -- just the 0xAfX is captured (however the parser rejects it because it matches number * space + expression)
})

test("hex", hex, {
    "0xfF",
    "0xfg",-- this returns 4; a capture would give 0xf, but per above parser rejects it
    "0xf",
    "0xfFa", 
    "x00",
})
...
number = (scientific + hex + decimal) / node * space -- node for front end
...
local grammer1 = lpeg.P{"relation",
  factor = number + OP * relation * CP,
  ...
  expo =  lpeg.Ct(suffix * opE * suffix) / foldBin  * space + suffix,
...
grammer1 = space * grammer1 * -1

-- try out mutliple grammers to see effects e.g. making zone lower priority, alt ways of making patterns, etc
local grammer2 = lpeg.P{"relation",
  factor = number + OP * relation * CP,
  prefix = factor + lpeg.Ct(((spaces + 0) * opP)^1 * factor) / foldPre,
  zone = lpeg.Ct(prefix * (opZ  * prefix)^0 ) / foldBin  * space,
...
local function parse(input, options)
  ...
  options.grammer = options.grammer and options.grammer or grammer1
...
local function run(code, stack, options)
  options = options and options or {}
...
--]]
print("\ntrace GRAMMER1")
pl({"5 - - -2"},{trace=true, grammer=grammer2})