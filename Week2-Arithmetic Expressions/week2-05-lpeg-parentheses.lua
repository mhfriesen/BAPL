...
local exponential = lpeg.V"exponential"
local term = lpeg.V"term"
local expression = lpeg.V"expression"

local grammer = lpeg.P{"expression",
  factor = number + OP * expression * CP * space,
  exponential = lpeg.Ct(factor * (opE * factor)^0) / foldBin  * space,
  term = lpeg.Ct(exponential * (opM * exponential)^0) / foldBin  * space,
  expression = lpeg.Ct(term * (opA  * term)^0) / foldBin  * space
}
grammer = space * grammer * -1

local function parse(input)
  return grammer:match(input)
end
