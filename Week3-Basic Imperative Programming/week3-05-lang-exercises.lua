...
function foldSuf2(lst)
  print("foldSuf2", type(lst))
  local tree = lst[1]
  for i=2, #lst do
    tree = { tag = "sufop", e1 = tree, op = lst[i] }
  end
...
local var = (ID + lpeg.C(keywords)) / nodeVar --TODO: don't capture decx etc

--print(keywords:match("decx"))
...
--not should be lower priority than relation
...
local grammer = lpeg.P{"stmts",
  ...
  stmt = OB * CB -- empty block
      + block 
      + (ID + keywords) * Assgn * notty / nodeAssgn -- allow keywords but trap in ast creation assgnNode
  ...
  factor = number + OP * notty * CP + var, --allow keywords in var but trap in ast creation varNode
  prefix = ( -#(var) * factor 
             + lpeg.Ct(
                          (opP^1 * factor )  
                        + (opPs^1 * factor)  
  ...
  -- alt from Roberto expo = suffix + (opE * expo)^-1 / foldExpo * space,
...
function pl(cases, options)
  ...
  for _,case in ipairs(cases) do   
    ...
    local stack = {}
    local mem = {}
    print("--->"..tostring(run(code, stack, mem, options)))
...
print("NEW cases")
pl({"x = not not - -2<3; return x"},{trace=true, grammer=grammer})
--pl({"x=1:3; y=dec 2; z= inc 4; @ (x+7); @ 2<3; @ not (2<3); return -x + y+z"},{trace=true, grammer=grammer})

--pl({"return 10 + inc 3:1"},{trace=true, grammer=grammer})
--pl({"de =dec dec 1; return-de"},{trace=true, grammer=grammer})
