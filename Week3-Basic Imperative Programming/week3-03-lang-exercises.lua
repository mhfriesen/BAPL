...
local opN = (lpeg.C(lpeg.P("not"))
            + lpeg.C(lpeg.P("dec"))
            + lpeg.C(lpeg.P("inc")) ) * space  --unuary left associative not like op
...
function foldLeft(lst)
  local tree = lst[#lst]
  for i = 2, #lst do
    tree = { tag = "nottyop", e1 = tree, op = lst[1] }
  end
  return tree
end
...
local grammer = lpeg.P{"stmts",
  ...
  stmt = OB * CB -- empty block
      ...
      + ID * Assgn * notty / nodeAssgn -- assignment
  ...
  notty = lpeg.Ct( (opN^0 * relation)^0 ) / foldLeft  * space
}
...
--repeatable prefix ops, spaces not allowed (would be confused with same symbol binary ops)
local preOps = {
  ["-"] = "neg",
}

-- repeatable not like ops, space mandatory
local nottyOps = {
  ["not"] = "not",
  ["dec"] = "decr",
  ["inc"] = "incr",
...
local function run(code, stack, mem, options)
  ...
  while true do 
    ...
    elseif code[pc] == "neg" then -- per above
      stack[top] = -stack[top]
    elseif code[pc] == "not" then -- per above
      stack[top] = not stack[top]
    elseif code[pc] == "decr" then -- per above
      stack[top] = stack[top] - 1
    elseif code[pc] == "incr" then -- per above
      stack[top] = stack[top] + 1
...
print("NEW cases")
pl({"return dec 3"},{trace=true, grammer=grammer})
