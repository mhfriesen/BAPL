...
local function nodeVar(var)
  print("\nnodeVar")
  print("var=",var)
  if keywords:match(var) then error("vars cannot be a reserved word " .. var) end
...
local notkeyword = -#(keywords * -alpha_numeric)

local identifier = --[[notkeyword *--]] (underscore + alpha) * (alpha_numeric + underscore)^0
local object = notkeyword * identifier * lpeg.P(".") * identifier
local ID = lpeg.C(identifier --[[+ object--]]) * space
local var = (ID + lpeg.C(keywords)) / nodeVar --TODO: ...................................get keywords capturing in nodeAssgn
                                                  -- i.e. work around or refactor notkeyword -  not capturing anything so id gets skipped

print((ID):match("dec"))
print((keywords+ID):match("dec"))
print((ID+keywords):match("dec"))
...
local opP = lpeg.C(lpeg.S("-+")) * space   --unuary left associative, spaces optional           
local opPs= lpeg.C(                        --unuary left associative, spaces mandatory (could be an ID otherwise)
            lpeg.P"not"       
                + "dec"
                + "inc"
                + "return"
...
local grammer = lpeg.P{"stmts",
  ...
  stmt = OB * CB -- empty block
      + block -- normal block of stmts TODO: would + 
      + (ID + keywords) * Assgn * relation / nodeAssgn -- TODO: allow kws, but trap in ast creation, kws should get to prefix with no =)
  ...
  factor = number + OP * relation * CP + var, --TODO: allow kws in var but trap in ast creation
  --prefix = -#(var) * factor + lpeg.Ct(lpeg.C("inc") * space * number) / foldPre,
  prefix = (-#(var) * factor + lpeg.Ct(
                          (opP^1 * factor )  
                        + (opPs^1 * space * factor)  
                    ) / foldPre )
          + factor,
  ...
  --relation = lpeg.Ct(lpeg.C("inc") * space * number) / foldPre
...
-- recursively generate code ast expressions
...
print"\nprimoridal pl (PPL) user cases\n"
print("NEW cases")
--pl({"return 10 + inc 3:1"},{trace=true, grammer=grammer})
pl({"dec =dec dec 1; return-de"},{trace=true, grammer=grammer})
--pl({"{x=3;;; y=1:2}; { }; return x+y"},{trace=true, grammer=grammer})

--[[
pl({"return 10 + decdec dec incinc+3:+1"},{trace=false, grammer=grammer})
pl({"return 10 + decdec dec incinc+(3:+1)"},{trace=false, grammer=grammer})
pl({"return 10 + decdec dec incinc3:1"},{trace=false, grammer=grammer})
