...
local new_capture = #(ID * space * T"="  * T"new") * -- is new array 
    function(s,p) pp = p; return true end * -- matchtime save ID position in pp
    lpeg.C(ID) *                            -- capture ID
    function(s,p)                           -- matchtime syntax/semantic checks                                    
      local id = string.sub(s, pp, p - 1)
      local isReserved = keywords[id]
      if isReserved then
        if not syntaxErr then M.report_syntax_error(s, p, "cannot assign to reserved word ".."'"..id.."'\n") end
...
local indexed_use = #(ID * space * T"[") * function(s,p) pp = p; return true end * lpeg.C(ID) * 
    function(s,p)
      ...
      local isReserved = keywords[id]
      local isArray = ast_vars[id]
      --print("indexed to use p="..tostring(p), "var=".."'"..id.."'", " isArray="..tostring(isArray))
      local msg = ""
      if not isArray then
        msg = msg .. "array '" ..id.. "' does not exist (must be newed first)"
        print("id", id, "ast_vars", pt.pt(ast_vars))
      end
      if isReserved then
...
-- foldNew returns {tag='variable', var='v'}, {tag='new', sizes = {{tag='number', size = s1} ... }
local function foldNew(lst) 
  --print("foldNew lst=", pt.pt(lst))
  local sizes = { } 
  for i = #lst, 2, -1 do
    sizes[#sizes + 1] = lst[i]
  end
  local tree = {tag = "assgn", lhs =  lst[1], expr = {tag = 'new', sizes = sizes}}
  --print("foldNew tree=", pt.pt(tree))
  return tree
end
...
local function foldIndexed_assgn(lst)
  ...
  --print("foldIndexed_assgn tree=", pt.pt(tree))
  return tree
end

local function var_assgn(ast)
  --print("var_assgn ast=", pt.pt(ast))
  local tree = {tag = "assgn", lhs = ast[1], expr = ast[2]}
  --print("var_assgn tree=", pt.pt(tree))
  return tree
end

local function foldIndexed_use(lst)
  --print("foldIndexed_use lst=", pt.pt(lst))
  ...
  --print("foldIndexed_use tree=", pt.pt(tree))
  return tree
end
...
local open = lpeg.P('"')
--local close = open
local close = -lpeg.B("\\") * open + open * -1 --TODO: use Matt's \x semantics implementation
...
local grammar = P{"prog",
  ...
  stmt = 
      ...
      -- assignments
      + Ct(new_capture * (T"[" * shortcct * T"]" )^1 ) / foldNew  
  ...
  factor = 
      ...
      + T"(" * shortcct * T")" 
      + Ct(indexed_use * (T"[" * shortcct * T"]")^0) -- {ID, shortcct1, shortcct2 ... shortcctn}
        / foldIndexed_use 
...
function Compiler:codeExp(ast)
  ...
  elseif ast.tag == "new" then  -- vm to put in mem associated with 
      --print("coding new ast sizes", #ast.sizes)
      for i = 1, #ast.sizes do
        --print(pt.pt(ast.sizes[i]))
  ...
  elseif ast.tag == "indexed" then -- vm mem[array][index] with array, index on stack
      --print("code=", pt.pt(Compiler.code))
      --print("coding getarray ast=", pt.pt(ast))
...
function Compiler:codeAssgn(ast)
  --print("codeAssgn code", pt.pt(Compiler.code))
  --print("codeAssgn ast", pt.pt(ast))
  local lhs = ast.lhs
  ...
  elseif lhs.tag == "indexed" then
    --print("code=", pt.pt(Compiler.code))
    --print("assgn indexed ast------", pt.pt(ast))
    --print("lhs.array", pt.pt(lhs.array))
...
local function run(code, stack, mem, options)
  ...
  while true do 
    ...
    elseif code[pc] == "newarray" then -- {size=dims[1], {size=dims[2]... dims[1]
      ...
      --print("dims", pt.pt(dims))
      local newedArray = create_new_array(1, dims)
      --print("newedArray", pt.pt(newedArray))
      top = top - n
