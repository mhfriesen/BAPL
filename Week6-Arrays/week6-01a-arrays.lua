...
local ast_arrays = {}     -- for saving arrays by index
...
local var_capture = #(ID) *                 -- is ID 
    --TODO: could split these into var/new/ind and add look ahead checks in each to reduce further processing and backtracking
    function(s,p) pp = p; return true end * -- matchtime save ID position in pp
    lpeg.C(ID) *                            -- capture ID
    function(s,p)                           -- matchtime save ID text in capturedVar                                    
      capturedVar = string.sub(s, pp, p - 1)
      return true
    end
 * space 
 
 
--local bracketAhead = #P(lpeg.space^0 * P"[")

-- var = expression
local var_assgn =  
    function(s,p)
      local id = capturedVar
      ...
      if isReserved then
        ...
        ast_vars[id] = true
        --print("ast_vars", pt.pt(ast_vars))
        --print("ast_arrays", pt.pt(ast_arrays))
        return true
      end
    end
 * space 
...
local new_assgn =  
    function(s,p)
      local newAhead = #P(lpeg.space^0  * P"new")
      if newAhead:match(string.sub(s, p)) then
        local id = capturedVar
        local isReserved = keywords[id]
        if isReserved then
          if not syntaxErr then M.report_syntax_error(s, p, "cannot assign to reserved word ".."'"..id.."'\n") end
          ...
          --print("new array p="..tostring(p), "var=".."'"..id.."'", "isReserved="..tostring(isReserved))
          ast_arrays[id] = true
          --print("ast_arrays", pt.pt(ast_arrays))
          --print("ast_vars", pt.pt(ast_vars))
          return true
...
local ind_assgn =  
    function(s,p)
      local id = capturedVar
      local isReserved = keywords[id]
      local isArray = ast_arrays[id]
      --print("var to use p="..tostring(p), "var=".."'"..id.."'", "isDefined="..tostring(isDefined), "isArray="..tostring(isArray))
      if isReserved then
        if not syntaxErr then M.report_syntax_error(s, p, "cannot assign to reserved word ".."'"..id.."'\n") end
        return false 
      elseif not isArray then
        if not syntaxErr then M.report_syntax_error(s, p, "array variable ".."'"..id.."' must be newed first\n") end
        return false
      end
      return true
    end
...
local var_use = #(ID) * function(s,p) pp = p; return true end * lpeg.C(ID) * 
    function(s,p)
      ...
      local var_exists
      local bracketAhead = #P(lpeg.space^0 * P"[")
      if bracketAhead:match(string.sub(s, p)) or isArray then
        type_ = "array"
        var_exists = isArray
      else
        type_ = "variable"
        var_exists = isDefined
      end
      if not var_exists then
        msg = msg .. type_ .. " '" ..id.. "'" .. " does not exist"
        if type_ == "array" then 
          msg = msg .. "\narrays need to be newed first"
        end
      end
      if isReserved then
         msg = msg .. "\ncould not redefine the reserved word ".."'"..id.."' anyway"
      end
      if not syntaxErr and (not var_exists or isReserved) then
...
local grammar = P{"prog",
  ...
  stmt = 
      ...
      + -- assignments
      ( var_capture * T"[" * ind_assgn 
          / node("variable", "var") * shortcct * T"]"                   -- a[x] =
          / node("indexed", "array", "index")  * T"=" * shortcct        
      + var_capture * T"=" * new_assgn                                  -- x = new [size]
          / node("variable", "var") * (Rw"new" * T"[" * shortcct * "]" 
          / node("new", "size"))    
      + var_capture * T"=" * var_assgn                                  -- x = a
          / node("variable", "var") * shortcct                         
      ) / node("assgn", "lhs", "expr")      
      + T"@" * shortcct / node("print","expr") -- printing
      + space, 
  block = T"{" * stmts * T";"^-1 * T"}",
  nested_elseifs = Rw"else" * block + Rw"elseif" * shortcct * block *  nested_elseifs^-1  / node("if_", "cond", "then_", "else_"),
  factor = 
        number / tonumber / node("number","val") 
      + strpat / node("string", "val")
      + T"(" * shortcct * T")" 
      + var_use * T"[" * shortcct * T"]" / node("indexed", "array", "index")
...
function Compiler:codeExp(ast)
  ...
  elseif ast.tag == "string" then
    -- in a real VM this might have to be an array of bytes, so probably need arrays first
    self:addCode("push")
    self:addCode(ast.val)
  ...
  elseif ast.tag == "indexed" then -- vm mem[array][index] with array, index on stack
    self:addCode("load")
    self:addCode(self:var2num(ast.array))
...
function Compiler:codeAssgn(ast)
  ...
  if lhs.tag == "variable" then
    --print("variable lhs", pt.pt(lhs))
    if ast.expr.tag == "new" then
      self:codeExp(ast.expr.size)
      self:addCode("push")
      self:addCode(self:var2num(lhs.var))
      self:addCode("newarray")   -- vm to put in mem associated with var2num, size on stack 
    else
  ...
  else error("unknown codeAssgn tag")
  end
end
...
local function run(code, stack, mem, options)
  ...
  while true do 
    ...
    elseif code[pc] == "getarray" then -- vm mem[array][index] with array, index on stack
      ...
      local t = stack[top - 1]
      top = top - 1 
      stack[top] = t.values[index]
    elseif code[pc] == "newarray" then -- allocate mem[num] = table[size] from stack num, size
      local varnum = stack[top]
      top = top -1 
      local size = stack[top]
      top = top -1 
      mem[varnum] = {size = size, values = {} }
    elseif code[pc] == "setarray" then -- mem[var2num][index] = expr from stack expr, index, table
      local expr = stack[top]
      top = top -1 
      local index = stack[top]
      top = top -1 
      local tab = stack[top]
      top = top -1 
      if tab.size < index then
        print("ast_arrays" , pt.pt(ast_arrays))
        print("mem" , pt.pt(mem))
        print("runtime error: array size=" .. tostring(tab.size) .. " < index=" .. tostring(index) )
        return nil
      else
        tab.values[index] = expr
      end
      --print("setarrary table", pt.pt(tab))
...
local function pl(cases, options)
  ...
  for i, case in ipairs(cases) do  
    ...
    --print("\nreport syntax error??? p_maxmatch="..tostring(p_maxmatch).. " case len="..tostring(#case ) )
