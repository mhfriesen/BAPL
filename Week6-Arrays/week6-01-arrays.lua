...
local function nodeVar(var)
  if keywords[var] then
    M.report_syntax_error(input, p_maxmatch, "keyword "..var.." cannot be used as a variable") 
    tree = {tag = "syntaxErr"}
  elseif ast_vars[var] == nil then 
    M.report_syntax_error(input, p_maxmatch, "variable "..var.." not defined") 
    tree = {tag = "syntaxErr"}
  else
...
local ID = lpeg.C(identifier + object) * -alpha_numeric * space
...
local var_use = I"varuse1" * #(ID) * I"varuse2" * function(s,p) pp = p; return true end * lpeg.C(ID) * 
    function(s,p)
      local id
      -- handle lhs, i.e., array variables
      print("capturedVar!", capturedVar)
      if capturedVar ~= "" then 
        id = capturedVar
      else
        id = string.sub(s, pp, p-1)
      end
      capturedVar = ""
...
local grammar = P{"prog",
  ...
  stmt = T"{" * T"}" 
      ...
      + lhs * T"=" * shortcct / node("assgn", "id", "expr") -- assgn
      + T"@" * shortcct / node("print","expr") 
      + space, 
  lhs = ID * T"[" * shortcct * T"]" / node("indexed", "array", "index")
      + ID,
  block = T"{" * stmts * T";"^-1 * T"}",
...
pl(take(cases,20, 20),{trace=true})
