...
-- n2 nil: return first n1 members of t
-- else    return n1 to n2 members of t
...
local function foldBin(lst)
  print("\nbin lst at p_maxmatch="..tostring(p_maxmatch)..":", pt.pt(lst))
  local tree = lst[1]
  for i=2, #lst, 2 do
    tree = { tag = "binop", e1 = tree, op = lst[i], e2 = lst[i+1] }
  end
  print("\nfolded bin tree at p_maxmatch="..tostring(p_maxmatch)..":", pt.pt(tree))
  return tree
end
...
local var_assgn =  
    function(s,p)
      ...
      if isReserved then
        M.report_syntax_error(s, p, "cannot assign to reserved word ".."'"..id.."'\n")
        return false 
      else
...
local var_use = #(ID) * function(s,p) pp = p; return true end * lpeg.C(ID) * 
    function(s,p)
      ...
      if (not isDefined) or isReserved then M.report_syntax_error(s, p, msg.."\n") end
      if not isDefined then
        return false 
      else
        return true
...
local grammar = P{"prog",
  ...
  zone = lpeg.Ct(prefix * (opZ  * prefix)^1 ) * I"Z" / foldBin  * space + prefix,
  suffix = lpeg.Ct(zone * opS^1) / foldSuf * space + zone,
  expo = lpeg.Ct(suffix * (opE * suffix)^1 ) / foldRight  * space + suffix,
  term = lpeg.Ct(expo * (opM * expo)^1 ) * I"T" / foldBin  * space + expo,
  expr = lpeg.Ct(term * (opA  * term)^1 ) / foldBin  * space + term,
  relation = lpeg.Ct(expr * (opR  * expr)^1 ) / foldBin  * space + expr,   
...
function M.parse(input, options)
  options = options or {}
  pp, p_maxmatch, ast_vars = 1, 1, {} --TODO: close pp, p_maxmatch, ast_vars, and closedVar in parse function
  syntaxErr = false
...
function Compiler:var2num (id)
  local num = self.state.vars[id]
  if not num then
    num = self.state.nvars + 1
    self.state.nvars = num
    self.state.vars[id] = num
  end
  return num
end
...
function pl(cases, options)
  ...
  for i, case in ipairs(cases) do  
    ...
    local ast, p_maxmatch, syntaxErr = M.parse(case, options)
