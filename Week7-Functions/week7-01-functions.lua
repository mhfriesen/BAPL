...
-- transform string with interpolation expressions e.g. "A {} with {} interpolations." & "string" & 1 + 1 ---> A string with 2 interpolations."
...
-- {a, b, c, array} where array.destructure could be in mt of all arrays (set when newing them)
local function multivar_assgn(lst)
  ...
  func_capture = #(ID * space * T"(") *     -- is name 
    ...
    function(s,p)                           -- matchtime syntax/semantic checks and register var                                    
      ...
      if isReserved then
        ...
        fwd_funcs[id] = true
        --ast_vars[id] = true -- make it a global name
        --ast_funcs[id] = true -- make it a func name
        --print("ast_funcs", pt.pt(ast_funcs))
...
function Compiler:codeExp(ast)
  ...
  elseif ast.tag == "binop" then
    ...
    elseif ast.tag == "interp" then
      -- transform string, expr, expr ... ----->  string1 | tostring(expr) | string2 ! tostring(expr) ...
      -- where string1, 2 ... come from {} {} that break up string as well as {expr1} {expr2} etc
      -- break string up
      -- 
    else
      self:codeExp(ast.e1)
...
--[[
to have recursion we need a way to call functions
]]
...
local optTrace, n1, n2 = false, 1, 7
cases = {
[[
