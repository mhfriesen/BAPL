...
local grammar = P{"prog",
  ...
  stmt = T"{" * T"}" -- empty block
      + block 
      + Rw"if" * notty * block / node("if_", "cond", "then_")
...
function Compiler:fixJmp2here(jmp)
  self.code[jmp] = jmp + (self:currentPosition() - self.code[jmp]) -- relative jump
end
...
function Compiler:codeStmt(ast)
  ...
  elseif ast.tag == "if_" then
    ...
    local jmp = self:codeJmp("jmpZ") --pc where jmp occurs
