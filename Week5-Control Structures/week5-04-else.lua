...
local grammar = P{"prog",
  ...
  stmt = T"{" * T"}" -- empty block
      + block 
      + Rw"if" * notty * block * (Rw"else" * block)^-1
...
function Compiler:codeJmp(op)
  self:addCode(op)
  self:addCode(0) -- to be updated later relative to the pos coded here
  return self:currentPosition()
end

function Compiler:fixJmp2here(jmp)
  print("jmp", jmp, "cur pos", self:currentPosition())
