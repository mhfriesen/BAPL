...
-- nest elseifcbs (this single function improves/simplifies previous version's two functions)
-- also, crucially, it just nests the ifelses...else so they can become the else of the if stmt
-- that enables the if statement to work like when it was just an if,then,else and be passed to node("if_" ...)
-- thus the if's else becomes any nested elseifs...else
-- HOWEVER since the improved func below suggests the nested elseifs can also use the node("if_" ...) 
-- it motivates using the grammar to do the recursion and as it unwinds feed node("if_" ...)
-- thus this function ultimately gets replaced by getting the grammar to do the recursion
-- this is just to show the reasoning
local function nest_elseifs(elseifcbs) 
  local cond, then_, else_
  if 1 == #elseifcbs[1] then    -- this corresponds to ^-1 recursive call in grammar
    if #elseifcbs[1] == 2 then  -- this corresponds to + alternative in the grammar
      cond = elseifcbs[1][1]
      then_ = elseifcbs[1][2]
      return {tag = "if_", cond = cond, then_ = then_}
    else
      then_ = elseifcbs[1]
      return then_[1]
    end
    ...
    cond = elseifcbs[1][1]
    then_ = elseifcbs[1][2]
    else_ = nest_elseifs(take(elseifcbs, 2, #elseifcbs))
    -- now as the recursion unwinds, nest the else_
    return {tag = "if_", cond = cond, then_ = then_, else_ = else_} -- this is what node("if_"...) does in the grammar
  end
end
...
local function Rw(word) return word * -#(alpha_numeric) * space end
...
local grammar = P{"prog",
  ...
  stmt = T"{" * T"}" 
      ...
      -- treat the nested_elseifs as the optional else block
      + Rw"if" * shortcct * block * nested_elseifs^-1 / node("if_", "cond", "then_", "else_")
      + Rw"return" * shortcct / node("return", "expr") 
      + var_capture * T"=" * var_assgn * shortcct / node("assgn", "id", "expr")
      + T"@" * shortcct / node("print","expr") 
      + space, 
  block = T"{" * stmts * T";"^-1 * T"}",
  -- nested_elseifs  = Ct(Ct((Rw"elseif" * shortcct * block))^0 * Ct((Rw"else" * block)^-1)) / nest_elseifs, -- manual nesting
  -- now using the grammar to do the nesting
  nested_elseifs = Rw"else" * block + Rw"elseif" * shortcct * block *  nested_elseifs^-1  / node("if_", "cond", "then_", "else_"),
  --                                node sees either alternative as the recursion unwinds; thus nesting them
  factor = number / tonumber / node("number","val") 
