...
-- nest ast.cond,then_ with elseifcbs 
...
-- nest elseifs into if else structure cbs = (cond block)s
...
local function nest_elseifs(elseifcbs) 
  ...
  if 1 == #elseifcbs[1] then
    local cond, then_, else_
    if #elseifcbs[1] == 2 then
      cond = #elseifcbs[1][1]
      then_ = #elseifcbs[1][2]
    ...
    return {tag = "if_", cond = elseifcbs[1][1], then_ = elseifcbs[1][2], else_ = else_}
  end
end
...
local grammar = P{"prog",
  ...
  stmt = T"{" * T"}" 
      + block
      + Rw"while" * shortcct * block  
      -- treat the nested_elseifs as the optional else block (where they need to be embedded)
      + Rw"if" * shortcct * block * nested_elseifs / node("if_", "cond", "then_", "else_")
      + Rw"return" * shortcct / node("return", "expr") 
      + var_capture * T"=" * var_assgn * shortcct / node("assgn", "id", "expr")
      + T"@" * shortcct / node("print","expr") 
      + space, 
  block = T"{" * stmts * T";"^-1 * T"}",
  -- like node(...) nest_elseifs will need to deal with nil params to uniformly nest either one,
  nested_elseifs  = Ct(Ct((Rw"elseif" * shortcct * block))^0 * Ct((Rw"else" * block)^-1)) / nest_elseifs,
  factor = number / tonumber / node("number","val") + T"(" * shortcct * T")" + var_use / node("variable", "var"), 
  ...
  notty =  Ct(opPs^1 * relation) / foldPre  + relation, --TODO: make dec and inc opPs notty for lower priority (unlike unarary minus
...
local grammar = P{"prog",
  ...
  stmt = T"{" * T"}" 
      ...
      + Rw"if" * shortcct * block * elifs / node("if_", "cond", "then_", "else_") 
      + Rw"return" * shortcct / node("return", "expr") 
      + var_capture * T"=" * var_assgn * shortcct / node("assgn", "id", "expr")
      + T"@" * shortcct / node("print","expr") 
      + space, 
...
--if_ = Rw"if" * shortcct * block * else_^-1  / node("if_", "cond", "then_", "else_"),
--else_ = Rw"else" * block   +   Rw"elseif" * shortcct * block * else_^-1  / node("if_", "cond", "then_", "else_"),
  elif  = Rw"elseif" * shortcct * block / node("if_", "cond", "then_", "else_"),
  elifs = Rw"else"  * block  + elif^0,
  --no matter which one we get, nest it into the else_ part of if's ast 
  --i.e. "elseif" takes place of "if" in node("if", "cond", "then_", "else_")
  --                                                                   ^
  --                                                         takes prev ast
  factor = number / tonumber / node("number","val") + T"(" * shortcct * T")" + var_use / node("variable", "var"), 
  ...
  notty =  lpeg.Ct(opPs^1 * relation) / foldPre  + relation, --TODO: make dec and inc opPs notty for lower priority (unlike unarary minus
  shortcct = lpeg.Ct(notty * (opL * notty)^1 ) / foldBin  * space + notty, 
  space = (lpeg.space + block_comment + comment)^0 * calc_maxp,
}
--]]
