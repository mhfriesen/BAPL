...
-- if, elseif, else transformer
-- transform if c1 b1 elseif c2 b2 ---> if c1 b1 else {if c2 b2} 
-- transform if c1 b1 elseif c2 b2 elseif c3 b3 else b4 ---> if c1 b1 else {if c2 b2 else {if c3 b3 else b4 3'}'   
-- transform if c1 b1 elseif c2 b2 .. else bn ---> if c1 b1 else {if c2 b2 else ..{if cn bn else bn+1 close n'}'  
local function nodeIf (tag, ...)
  local labels = table.pack(...)
  return function (...)
    local params = table.pack(...)
    local t = { tag = tag}
...
local grammar = P{"prog",
  ...
  stmt = T"{" * T"}" -- empty block
      + block 
      + Rw"if" * notty * block * (Rw"elseif" * block)^0 * (Rw"else" * block)^-1
...
function Compiler:codeStmt(ast)
  ...
  elseif ast.tag == "if_" then
    if ast.else_ == nil then
      ...
      self:addCode(self:var2num("_cond_"))
      self:addCode("load")      
      self:addCode(self:var2num("_cond_")) -- keep on stack for jmp
      local jmp = self:codeJmp("jmpZ")     -- jmp decision based on cond
      self:codeStmt(ast.then_)
      jmp = self:codeJmp("jmpZ")           -- add skip else jmp cond
      self:addCode("load")      
      self:addCode(self:var2num("_cond_")) 
      self:addCode("not")     
      self:fixJmp2here(jmp)               -- fix skipping past else_  
...
function M.compile(ast)
 ...
 Compiler.state = {code = {}, vars = {}, nvars = 0}
...
print"\n cleaned up primordial pl (aka CUP) user cases\n"
print("NEW cases")
