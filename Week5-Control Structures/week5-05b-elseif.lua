...
local function nest(ast, elseifcbs)  
 ...
 if #withs < 1 then
    ...
    ast = {tag = "if", cond = tn[1], then_ = {{t[2]}}} 
    print("\n init AST")
    print( pt.pt(ast))
    for i = #tn, 1, -2 do
      prepend(ast.then_, {{tag = "if_", cond = tn[i-2], then_ = {tn[i]}}})
      print("prepended AST")
      print( pt.pt(ast))
    end
    print("\n nested AST")
...
local grammar = P{"prog",
  ...
  stmt = T"{" * T"}" -- empty block
      + block 
--    + Rw"if" * notty * block * (Rw"else" * block)^-1 / node("if_", "cond", "then_", "else_") 
      + Rw"if" * lpeg.Ct(lpeg.Ct(notty * block  * elifs) * (Rw"else" * block)^-1) / nest_elifs --/ node("if_", "cond", "then_", "else_")
--                                ^ elseif sequence      
